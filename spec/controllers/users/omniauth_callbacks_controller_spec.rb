  ...
  describe 'GET complete' do
    context 'with valid new user' do
      it 'creates new user and new identity' do
        ...
        expect { get :complete, params: { provider: 'CCID' } }.to change { User.count }.by(1)
        user = User.last
        identity = user.identities.last
        expect(user.display_name).to eq 'John Doe'
        expect(user.email).to eq 'johndoe@ualberta.ca'
        expect(identity.provider).to eq 'CCID'
        expect(identity.uid).to eq 'johndoe'
        expect(controller).to set_flash[:notice].to(I18n.t('devise.omniauth_callbacks.success', kind: 'CCID'))
    ...
    context 'with valid existing user' do
      it 'uses existing identity if present' do
        user = create(:omniauth_user, email: 'johndoe@ualberta.ca', provider: 'CCID', uid: 'johndoe')
        ...
        expect { get :complete, params: { provider: 'CCID' } }.not_to change { User.count }
        identity = user.identities.last
        expect(user.email).to eq 'johndoe@ualberta.ca'
        expect(identity.provider).to eq 'CCID'
        expect(identity.uid).to eq 'johndoe'
